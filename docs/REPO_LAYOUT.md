...
| **`store/products/`** | Canonical product-ID galleries: WebP images, ordered manifests, and source hashes |
...
| Path | Why |
|------|-----|
| **`public/shop/`**, **`public/console/`**, **`public/store/`** | Generated/runtime trees. `public/store/products/_incoming` and `_completed` are local operator staging/archive storage and are ignored by git. |
| **`dist/`** | Build output |
...
2. **`npm run sync:store`** → recreates **`public/shop/`** and mirrors canonical galleries to **`public/store/products/`** locally
3. **`npm run build:pages`** → sanitized **`dist/`**, including only gallery WebPs/manifests under `dist/store/products/`, then GitHub Pages (**gear.aerovista.us**)
...
| **`docs/FOLDER_DUPLICATES.md`** | Canonical vs duplicate folder tags |
| **`docs/store-internal/handoffnotes.md`** | Short dev cheat sheet |
